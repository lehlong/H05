...
namespace H05.API.Controllers.WH
{
    ...
    [ApiController]
    public class MtypeController(IMtypeService service) : ControllerBase
    {
        public readonly IMtypeService _service = service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] MtypeDto time)
        {
            var transferObject = new TransferObject();
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] MtypeDto time)
        {
            var transferObject = new TransferObject();
        ...
        public async Task<IActionResult> Export([FromQuery] BaseMdFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Danh sách loại vật tư" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;
