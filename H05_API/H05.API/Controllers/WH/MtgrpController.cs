...
namespace H05.API.Controllers.WH
{
    ...
    [ApiController]
    public class MtgrpController(IMtgrpService service) : ControllerBase
    {
        public readonly IMtgrpService _service = service;
        ...
        [HttpPost("Insert")]
        public async Task<IActionResult> Insert([FromBody] MtgrpDto time)
        {
            var transferObject = new TransferObject();
        ...
        [HttpPut("Update")]
        public async Task<IActionResult> Update([FromBody] MtgrpDto time)
        {
            var transferObject = new TransferObject();
        ...
        public async Task<IActionResult> Export([FromQuery] BaseMdFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Danh sách nhóm vật tư" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;
