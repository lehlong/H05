...
namespace H05.API.Controllers.MD
{
    ...
    [ApiController]
    public class NotiTypeController(INotiTypeService service) : ControllerBase
    {
        ...
        public async Task<IActionResult> Export([FromQuery] BaseMdFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Danh sách loại thông báo" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;
