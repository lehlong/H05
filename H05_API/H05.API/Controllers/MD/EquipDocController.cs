...
namespace H05.API.Controllers.MD
{
    ...
    [ApiController]
    public class EquipDocController : ControllerBase
    {
        private readonly IEquipDocService _service;

        public EquipDocController(IEquipDocService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> Upload(IFormFile file, string equnr, string? docType = null)
        {
            var transferObject = new TransferObject();
            ...
            var result = await _service.UploadDocumentAsync(file, equnr, docType);
