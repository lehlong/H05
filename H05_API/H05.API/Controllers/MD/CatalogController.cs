...
namespace H05.API.Controllers.MD
{
    ...
    public class CatalogController(ICatalogService service) : ControllerBase
    {
        ...
        {
            var transferObject = new TransferObject();
            time.Id = time.CatCode + "-" + time.CatType + "-" + time.Code;
        ...
        public async Task<IActionResult> Export([FromQuery] BaseMdFilter filter)
        {
            ...
            if (_service.Status)
            {
                return File(result, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Danh sách bộ thống kê sự cố" + DateTime.Now.ToString() + ".xlsx");
            }
            else
            {
                transferObject.Status = false;
